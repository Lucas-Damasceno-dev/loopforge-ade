...
| M-04 | **CORS configurável** (`LF_CORS_ORIGINS`, CSV) com fim de `allow_origins=["*"]` com `allow_credentials=True`; **default mantido `*`** (wildcard — sem credentials) | Combinação atual é inválida para browsers; E5 mandava restringir; o default `*` foi mantido na implementação (compat dev solo) e a restrição passa a existir quando `LF_CORS_ORIGINS` é definida | B, C | A |
...
| M-21 | **Fila de execução (E3) real**: até `runner.max_concurrent_runs` (default 2) ativas + fila `queued`; `POST /runs` não dispara `asyncio.create_task` quando o limite está cheio; worker promove a próxima ao término | E3 é Tier-1 prometido (00/03/08) e **não existe**: hoje todo POST dispara task concorrente (`app.py:165-175`) — paralelismo não intencional, exatamente o que E3 queria evitar; SPA já modela `queue` no `runsStore` | B, C (status novo `queued`) | A (task A8) |
...
| M-10 | **Hard-stop = pausa (`paused`) + `POST /api/v1/runs/{id}/cost/override`**; aviso de 80% via campo `budget_warning` do `GET /cost` (não é evento WS) | Abort destrutivo desperdiça checkpoints; UX13 descreve pausa + override | B, C | A (backend) / D (UI real) |

## HITL e time-travel (features centrais do MVP)

| ID | Mudança | Justificativa | Impacto | Fase |
|---|---|---|---|---|
| M-11 | **`hitl.on_timeout: continue\|abort\|pause`** (default `continue` — compat; `abort` fail-closed explícito) | F1-13 (`continue`) é fail-open num feature de governança; `AGENTS.md` diz `abort` (drift); UX10 diz "permanece pausada". Config resolve; ADR-0006 propôs `pause` como default, a implementação manteve `continue` por compatibilidade e adicionou `abort` | B, C, D | C |
...
| ID | Mudança | Justificativa | Impacto | Fase |
|---|---|---|---|---|
| M-15 | **Pacote pip único** (ADR-0001): SPA vendored em `lf/src/lf/ade/static/`; **`pyproject.toml` do repo ADE mantido como placeholder** do pacote `loopforge-ade` (0.1.0, dist vazio — o embed real é `lf.ade.static.dist` via `sync_dist`); script `sync-dist` | E1 dizia "um pacote, uma versão" mas criava dois pacotes com dependência `file://` não publicável; o placeholder preserva o namespace do repo ADE | B, S (packaging), D | B |
...
| HITL timeout `continue` | spec Fase 1 F1-13 (aprovada) | Configurável (`continue\|abort\|pause`); default na implementação: `continue` (ADR propôs `pause`; M-11 manteve `continue` por compat + `abort` fail-closed) | ADR-0006, M-11 |
...
- Fila com até `max_concurrent_runs` (default 2) em paralelo (E3); atomic task
  checkout fica no V2.

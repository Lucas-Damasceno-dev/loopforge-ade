...
| A6 | Budget fonte única (`ade.yaml`→CircuitBreaker); `llm_costs` +`run_id,node,estimated`; `/runs/{id}/cost`; custo estimado subprocess; hard-stop pausa + `budget-override` + `budget_warning` | M-08, M-09, M-10 |
| A7 | Cleanup: TS morto em `src/`, `AGENTS.md`, nota mypy, doc `checkpoints.sqlite` legado | M-17 |
| A8 | **Fila de execução (E3)**: hoje todo `POST /runs` dispara `asyncio.create_task` concorrente (`app.py:165-175`) — paralelismo não intencional. Implementar 1 run ativa + fila: run nova vira `queued` quando há run `running`; worker promove a próxima `queued` ao término (eventos `run_updated` alimentam as abas/fila da UI) | M-21 |
...
| B5 | Pacote único: `sync-dist` script, `lf.ade.static` package-data, remoção do `pyproject.toml` do repo ADE; **criar** job de CI de drift SPA×static (hoje **não existe** — o engine só tem workflows Python: ruff/mypy/pytest) e **automatizar o smoke de instalação limpa** (CI: `pip install .` em venv limpo + `lf serve` + `GET /health` + `GET /app`) | M-15 |
...
| C4 | `hitl.on_timeout` (schema, dispatcher re-aguarda em `pause`); evento `hitl_gate_reached`; banner/badge na UI | M-11 |
...
run↔thread, **fix do HITL remoto e fila E3 — ambos hoje inexistentes/quebrados**,

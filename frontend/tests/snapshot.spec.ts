...
test('create-run com pipeline selecionado abre modal de snapshot', async ({ page }) => {
  ...
  await expect(page.getByText('Pipeline Snapshot').first()).toBeVisible()
  await page.getByLabel('Snapshot description').fill('desc e2e')
  await page.getByText('Create run').click()
})
